...
如果应用基于 Qt、GTK 等成熟的 GUI 框架开发，不直接使用 X11 API，那么在大多数情况下可以自动适配 Wayland，无需对应用本身做出修改。然而，在实际开发过程中，直接使用 X11 API 的情况并不少见。例如我此前开发的 AiOffice 应用，为了更精细地处理按键消息，绕过了 Qt，直接调用了 X11 的键盘事件相关接口。再比如 Wine，为了高效、准确地模拟 Windows API，也没有采用 Qt、GTK 这类重量级框架，而是直接构建在 X11 之上。对于这类应用而言，迁移到 Wayland 并不存在“自动适配”的捷径，而是必须进行专门的 Wayland 适配工作。

在查阅 Wayland 相关开发资料，尤其是 Wayland 客户端应用开发资料时，会发现可参考的内容相当有限。几乎翻遍网络之后，才找到一份质量尚可的教程——Writing Wayland Clients。遗憾的是，该教程托管在 GitBook 平台上，可能由于平台问题，部分章节已经无法访问。原作者也曾在网上吐槽这一情况，看起来连作者本人也未能完整保留原稿。
